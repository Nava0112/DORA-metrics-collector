...
def get_logs():
    try:
        ...
        return jsonify({
            "deployments": [
                {"deployment_id": d[0], "repo_id": d[1], "environment": d[2], "status": d[3], "created_at": d[4].isoformat()}
                for d in deployments
            ],
            "pull_requests": [
                {"pr_id": p[0], "repo_id": p[1], "merged_at": p[2].isoformat() if p[2] else None, "base_branch": p[3]}
                for p in prs
            ],
            "incidents": [
                {"issue_id": i[0], "repo_id": i[1], "created_at": i[2].isoformat(), "closed_at": i[3].isoformat() if i[3] else None, "is_incident": i[4]}
                for i in incidents
            ]
        }), 200
...
def calculate_metrics():
    try:
        process_metrics()
        return jsonify({"status": "Metrics recalculated successfully"}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
